...
export class ChannelService {
  ...
  async sendMobileMoneyVerificationCode(id: string): Promise<GenericResponse> {
    ...
    const smsUrl = this.configService.getOrThrow<string>('SMS_URL');
    const smsSecret = this.configService.getOrThrow<string>('SMS_SECRET');

    const generateSixDigitPIN = (): string => {
      return Math.floor(100000 + Math.random() * 900000).toString();
    };

    const message = `Channel verification code: ${generateSixDigitPIN()}. This code will expire in 10 minutes.`;

    console.log('sms config', smsUrl, smsSecret);
